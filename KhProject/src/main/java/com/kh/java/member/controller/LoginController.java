...
public class LoginController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(loginMember != null) {
			...
			// 지금은 포워딩방식으로 응답데이터를 보냈다, 다른 방식으로도 해보자.
			// 첫번째로 index.jsp로 응답데이터를 만들어 보내는데, 포워딩방식을 항상 사용한다고 가정하면
			// 불편함이 생길 수 있음
			// 응답해주는 응답 jsp파일의 이름이 다른 파일로 대체될 수 있음
			// 웰컴파일을 index.jsp말고 다른걸로 바꾸자, 해서 web.xml에서 변경을 하고 새로 만드는 식으로 파일 자체가 변경
			// 이러면 이게 바뀔때마다 코드에서 같이 변경해줘야함(getRequestDispatcher 인자값 변경이 번거로움)
			// 스텝 2 주석처리함
			
			// 포워딩해서 받을수도 있지만, 클라이언트가 이 웰컴파일을 받을 수 있는 다른 방법이 있음
			// localhost:4000/kh 이렇게 요청 보내면 웰컴파일이 온다(index.jsp)
			// 클라이언트가 이렇게 요청을 보내게끔 하는 방법
			// 이미 이 경로를 받을 수 있는 url이 있으니까, 그 경로로 다시 요청하게끔 하는 방법
			// sendRedirect : Client에게 URL을 다시 요청하게 함
			// 지금 포워딩방식은 뒤에 login이라고 url에 남아있었음! 이거기억
			...
			/*
			 * 무슨 차이인가?
			 * index.jsp를 index2.jsp로 웰컴파일을 변경한다면
			 * 웰컴파일이 index2로 나온다
			 * 포워딩은 index.jsp로 가는데, 지금은 index2.jsp가 응답이 온다
			 * 그래서 그때마다 request dispatcher 수정이 필요없는것
			 * 지금은 요청이 두 번 일어난 상황
			 * 로그인 요청을 보내면? /kh/login으로 요청이 보내짐
			 * 서버는 요청을 받았으니 응답을 해줘야하는데, response 내용을 응답 받고싶으면 /kh로 다시 요청 보내봐, 그럼 응답받을수있어 라고 서버가 응답해줌
			 * 이렇게 클라이언트에게 알려주면 클라이언트가 서버가 알려준 url로 다시 요청을 보냄, 302
			 * 그럼 서버가 index2로 응답을 해준다
			 * sendRedirect는 클라이언트가 요청을 다시 보낼 곳을 알려주는것
			 * 
			 * 공무원식 일처리 방식이라고 생각
			 * 로그인해서 index.jsp를 받아야하는데, 그걸 받고싶으면 /kh에 요청을 또 하라고 하는거임
			 * 그러면 index.jsp를 받을 수 있으니 거기로 요청을 보내라 했으니 다른 url로 요청을 보내는것
			 * 
			 * sendRedirect : 요청 보낼 url을 알려주는 방식
			 * 
			 */
