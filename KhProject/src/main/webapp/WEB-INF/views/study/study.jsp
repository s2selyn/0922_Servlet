...
<body>
		...
		function memberInfo() {
			...
			$.ajax({
				...
				success : function(response) {
					...
					// JSON으로 보내는걸로 수정하고 출력(JSON 배열을 사용하는 방법)
					...
					// 읽기 편해지고 코드를 파악하는 속도가 빨라지니 유지보수가 용이해짐
					// 일반적으로 여러 값을 다룰때는 key값이 달린 자료형을 선호한다
					// 효율로 따지면 배열이 더 좋을수는 있지만, key값을 사용하는것이 가독성, 유지보수, 실수줄이기에 좋다
					
					// 같이 작업하는 사람이 있을 때는 작업 스타일을 동일하게 맞추는 것이 가장 효율적이다
					// 뭔가 다른 방법을 하고싶다면 설명하고 배우는 과정이 필요해짐
					// 나혼자하고 나혼자볼거면 상관없지만 코드는 대부분이 여럿이 개발함, 1인개발은 불가능한 수준
					// 남에게 설명도 잘해야하지만 남이 봤을때 이해하기 어렵다면 생산성이 떨어지겠지
					// 정답은 존재하지 않지만 내가 만들고 설명을 못한다면 애매할지도... 코드는 쉽게 쓰는 게 최고야 누구나 쉽게 한눈에 알아볼수있다면 그게 좋겠지
					// 메모리나 동작이 늘어나더라도 우리는 아직 그런 극단적인걸 할 단계가 아님, 그런 고민은 개발자로서 좋은 마음가짐이긴 하지만 다른사람들과 같이 결과물을 만들어내는 단계에서는 조심해야함
					
				},
				
				error : function(e) {
					
					console.log(e); // statusText: "parsererror" -> JS가 파싱 못해서 난 에러라고 함
					// JS라면 그냥 이걸 사용하면 된다
	...
	<%-- 그리고 지금은 SQL문으로 처리하는것이 제일 효율적, DBMS가 그걸 위한 프로그램, for문보다 SQL로 해결하는게 더 효율적(DECODE 쓰는 그런식) --%>
	
	<h3>3. 서버로 요청 후, 여러 개의 객체 응답받아 출력해보기</h3>
	
	<%-- 아이디를 입력받아서, 입력받은 아이디로 작성된 게시글을 전부 조회하는 기능을 구현해보자, 마이페이지에서 내가 쓴 글 보는 그런 기능이겠지 --%>
	<%-- 임의로 서블릿에서 더미게시글 정보를 보내는 것으로 해보자, DB갔다오기 시간 너무걸림 --%>
	...
	<script>
		...
		function find() {
			...
			// 게시글 조회하려면? DB 갔다와야함, 할 수는 있겠지 스크립틀릿 써서.. 근데 그러고 싶지 않거든요?
			// 이걸 누구한테 해달라고 하나여? 컨트롤러 역할을 하는 서블릿, 비동기식 요청으로 보낼것이므로 jQ를 이용한 AJAX
			$.ajax({ // 보낼 값이 많다, URL, 전송방식, 성공 시 수행할 일... -> 객체로 만들어서 보냄
