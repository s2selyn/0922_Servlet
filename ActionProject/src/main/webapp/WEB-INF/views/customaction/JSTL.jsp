...
<body>
		...
		<tbody>
			<c:choose>
				...
				<c:otherwise>
					...
					순서를 붙여주고 싶을 때가 있다면?
					tr 자식으로 th 태그 하나 추가하고 forEach에 속성추가, 값은 아무거나 써도됨, 변수 하나 추가하는 개념
					status 따서 s로 보통 작성하는데
					s의 속성 여러가지 중에 index가 있음 이건 0번부터 시작
					순번인데 0부터 시작 이상하니까 count 사용 이건 1번부터 시작
					--%>
	...
	<%-- 데이터 정리해서 보여줄때 사용하는 태그, 마지막에 통계 이런거 출력해주는 경우가 있음, 표에 대한 정보, 총합 등
	이런걸 입력하는 영역이 tfoot, 레이아웃이 바뀌는 것은 아니고 검색했을 때 각각 총합 검색은 tfoot에서 하도록함
	최적화 위해서 사용하는 태그
	검색엔진최적화(SEO), 상위노출 등
	모든 회사들의 목표, 검색했을 때 자기네들것이 위에 나오고싶음
	웹사이트는?
	개발하는 입장에서 생각하면 정보제공
	기업, 세일즈 입장에서는 판매하는 것을 안내, 홍보하는 역할
	태그를 잘 나누어서 쓰는 것이 기초적이고 중요한 작업
	신입개발자가 개발잘하는걸 어케 증명해, 같이 일해봐야알지
	비즈니스 마인드 등의 개발 외적인 부분도 나만의 생각을 가지자
	--%>
	...
	<%-- forTokens 내부에 웬만하면 다른 태그 넣지 말고 출력문만, 문제가 생길 여지가 있음 --%>
	
	<!-- 테스트할 값 -->
	<c:set var="device" value="컴퓨터,핸드폰,TV/에어컨.냉장고-세탁기" />
	<%-- 구분자가 여러개인 상황, 배운걸로 생각해보면? 반복을 돌리면서 조건써서 정규표현식으로 분리시키면 가능하겠지만 그거자체가 번거로움 --%>
	<%-- 이럴 때 forTokens로 간편분리가능
	var는 항상 내가 쓰고싶은 제어변수명
	분리하고자하는 문자열을 EL구문으로 작성
	delims에 구분자로 쓸거 다 넣어버림, 이걸 이용해서 알아서 분리해준다
	
	자주 쓰는건 아니고 간혹 어쩌다 한번 이런 상황이 생길 수 있음
	--%>
	<ul>
	...
	<%-- 뭔가 더 많다, 날짜 포매팅, 함수제공 라이브러리 등 여태 한 만큼의 분량 정도
	근데 뷰의 역할은 화면에 뭔가 출력, 값 입력받는거니까 여기서 연산이나 포매팅 하는거아님 빠지는게 좋다
	DB에 있는 내용을 화면 출력에 바꾸고싶다면 이건 SELECT할때 바꾸는게 맞다
	function라이브러리는 EL에서 자바의 메소드를 쓸 수 있으니 굳이 쓸 필요가 없음
	그래서 결국 안하기로 결정
	--%>
