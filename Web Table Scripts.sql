...
CREATE TABLE KH_REPLY(
  REPLY_NO NUMBER PRIMARY KEY, -- sequence로 값을 만들어서 넣을것
  REPLY_CONTENT VARCHAR2(400) NOT NULL, -- 댓글의 내용
  REF_BNO NUMBER NOT NULL, -- 어느 게시글에 달릴지
  REPLY_WRITER NUMBER NOT NULL, -- 누가 작성했는지
  CREATE_DATE DATE DEFAULT SYSDATE NOT NULL, -- 보통 최신순으로 보이니까 정렬용으로
  STATUS VARCHAR2(1) DEFAULT 'Y' CHECK (STATUS IN ('Y', 'N')),
